...
class PlottingApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("多功能繪圖工具-V2.5")
    ...
    def init_ui(self):
        ...
        self.x_tick_label_bold_checkbox.toggled.connect(self.update_plot_with_timer)
        ...
        self.y_tick_label_bold_checkbox.toggled.connect(self.update_plot_with_timer)
    ...
    def update_plot(self):
        ...
        if len(colors_to_plot) != len(y_to_plot):
            colors_to_plot = [self.plot_color_hex] * len(y_to_plot)
        ...
        if self.box_checkbox.isChecked():
            # 盒鬚圖不使用 X 軸，直接繪製 Y 數據
            box_plot = self.ax.boxplot(y_to_plot, patch_artist=True)
            box_color = colors_to_plot[0] if colors_to_plot else self.plot_color_hex
        ...
        if self.show_data_labels_checkbox.isChecked():
            if not self.box_checkbox.isChecked():
                ...
                # 盒鬚圖的數據標籤通常是中位數
                if y_is_numeric:
                    median_val = np.median(y_to_plot)
        ...
        self.ax.tick_params(axis='x', labelsize=self.x_tick_label_size_spinbox.value(), direction=selected_tick_direction,
                            labelweight='bold' if self.x_tick_label_bold_checkbox.isChecked() else 'normal')
        self.ax.tick_params(axis='y', labelsize=self.y_tick_label_size_spinbox.value(), direction=selected_tick_direction,
                            labelweight='bold' if self.y_tick_label_bold_checkbox.isChecked() else 'normal')
        ...
        if self.x_interval_spinbox.value() > 0 and x_is_numeric and not self.box_checkbox.isChecked():
        ...
        if self.minor_x_interval_spinbox.value() > 0 and x_is_numeric and not self.box_checkbox.isChecked():
