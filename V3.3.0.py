...
class PlottingApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("多功能繪圖工具-V3.3.2") # 版本號更新
    ...
    def update_plot(self):
        ...
        for dataset_index, dataset in enumerate(self.datasets):
            ...
            if self.line_checkbox.isChecked() or (self.scatter_checkbox.isChecked() and self.connect_scatter_checkbox.isChecked()):
                if x_is_numeric and y_is_numeric and SCIPY_AVAILABLE and self.smooth_line_checkbox.isChecked():
                    try:
                        ...
                        print(f"無法生成平滑曲線: {e}")
                        line_artist, = self.ax.plot(x_data, y_data, color=primary_color, linewidth=linewidth,
            ...
            if self.bar_checkbox.isChecked():
                if x_is_numeric:
                    bars = self.ax.bar(range(len(x_data)) if not x_is_numeric else x_data, y_data, width=self.bar_width_spinbox.value(), 
                                       color=colors, edgecolor=border_color, linewidth=self.border_width_spinbox.value(), zorder=2, label=name)
                    if not x_is_numeric: self.ax.set_xticks(range(len(x_data)), x_data)
                    for rect in bars: self.artists_map[rect] = {'dataset_index': dataset_index, 'type': 'bar'}
                    legend_handles.append(bars[0])
                    legend_labels.append(name)
                else:
                    print("長條圖需要數值型X軸或類別型X軸。")
                ...
                if not self.box_checkbox.isChecked():
                    for i, (x, y) in enumerate(zip(x_data, y_data)):
                        ...
                        if label_text:
                            ...
                            annot_x = i if not x_is_numeric and self.bar_checkbox.isChecked() else x
        ...
        all_x_numeric = all(isinstance(x, (int, float)) for ds in self.datasets for x in ds['x'])
            ...
            if contains and (artist_info := self.artists_map.get(artist)):
                ...
                try:
                    ...
                    if point_idx != -1:
                        ...
                        self.plot_color_hex = current_ds.get('primary_color', '#1f77b4')
    ...
    def pick_color(self, target):
        initial_color = getattr(self, f"{target}_color_hex", self.plot_color_hex)
        ...
        if color.isValid():
            ...
            if target == "plot":
                if self.selected_artist_info and (ds_index := self.selected_artist_info['dataset_index']) < len(self.datasets):
                    ...
                    ds['line_segment_colors'] = [hex_color] * (len(ds['x']) - 1)
                    self.plot_color_hex = hex_color
                else: 
                    self.plot_color_hex = hex_color
                    self.update_dataset_colors(hex_color, update_all=True)
                self.update_table()
            elif target == "border":
                if self.selected_artist_info and (ds_index := self.selected_artist_info['dataset_index']) < len(self.datasets):
                ...
                setattr(self, f"{target}_color_hex", hex_color)
    ...
    def update_data_from_table(self):
        ...
        for i in range((self.data_table.columnCount() - 1) // 2):
            ...
            colors = [self.data_table.item(r, c_col).background().color().name() if self.data_table.item(r, c_col) else self.plot_color_hex for r in range(self.data_table.rowCount())]
        ...
        try:
            ...
            for y_col in y_cols:
                ...
                self.datasets.append({
                    ...
                    'colors': [self.plot_color_hex] * n, 'primary_color': self.plot_color_hex, 
                    'line_segment_colors': [self.plot_color_hex] * (n - 1), 'marker': '圓形', 
    ...
    def add_row(self):
        if not self.datasets:
            self.datasets.append({'name': '數據1', 'x': [0], 'y': [0], 'colors': [self.plot_color_hex], 'primary_color': self.plot_color_hex, 'line_segment_colors': [], 'marker': '圓形', 'border_color': '#000000', 'linewidth': self.line_width_spinbox.value(), 'linestyle': self.linestyle_combo.currentText()})
        else:
            for ds in self.datasets:
                last_x = ds['x'][-1] if ds['x'] and isinstance(ds['x'][-1], (int, float)) else len(ds['x']) -1
                ds['x'].append(last_x + 1); ds['y'].append(0); ds['colors'].append(ds['primary_color'])
        self.original_datasets = [ds.copy() for ds in self.datasets]
    ...
    def remove_row(self):
        ...
        for row in rows:
            for ds in self.datasets:
                if row < len(ds['x']):
                    for key in ['x', 'y', 'colors']: del ds[key][row]
        self.original_datasets = [ds.copy() for ds in self.datasets]
        self.update_table(); self.update_plot()
    ...
    def get_settings(self):
        settings = {w.objectName(): w.text() if isinstance(w, QLineEdit) else w.value() if isinstance(w, (QSpinBox, QDoubleSpinBox)) else w.isChecked() if isinstance(w, QCheckBox) else w.currentText() if isinstance(w, QComboBox) else None for w in self.findChildren((QLineEdit, QSpinBox, QDoubleSpinBox, QCheckBox, QComboBox))}
        settings.update({k: getattr(self, k) for k in self.findChildren(str) if k.endswith("_hex")})
...
if __name__ == '__main__':
    try:
        ...
        QMessageBox.critical(None, "啟動失敗", f"缺少函式庫: {e}")
    except Exception as e:
        QMessageBox.critical(None, "程式啟動錯誤", f"發生未預期的錯誤: {e}")
